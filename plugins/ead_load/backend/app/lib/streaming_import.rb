...
class StreamingImport
  ...
  def initialize(stream, ticker, import_canceled = false,  migration = false)
    ...
    @ticker = ticker
    @ticker.log("start of initialize")
  ...
  def do_create(record, noerror = false)
    begin
      ...
      @ticker.log("Creating: #{title_or_fallback(record)}")
